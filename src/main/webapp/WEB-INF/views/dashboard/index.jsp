                            ...
                            <div class="card-header py-3">
                                <h6 class="m-0 font-weight-bold text-primary">Enquetes em Votação:
                                    <c:if test="${not empty categoriasAberta.content}">
                                        <br><small>Exibindo: ${categoriasAberta.content.size()}
                                        de ${categoriasAberta.totalElements}</small>
                                    </c:if>
                            ...
                            <div class="card-body">
                                ...
                                <c:if test="${not empty categoriasAberta.content}">
                                    <c:forEach var="categoria" items="${categoriasAberta.content}">
                                        <fmt:formatNumber var="porcentagem"
                                                          value="${(categoria.votos.size() / (totalUsuariosEmpresa * 3) ) * 100}"
                                                          maxFractionDigits="0"/>
                                        <h4 class="small font-weight-bold"><a
                                                href="${context}/dashboard/campanhas/${categoria.campanha.id}/categorias/${categoria.id}"
                                                class="text-success">${categoria.campanha.nome}
                                            - ${categoria.tipo.descricao}</a>
                          ...
                          <c:if test="${porcentagem eq 'NaN'}">Total de Votos: 0</c:if>
                                        ...
                                        <div class="progress mb-4">
                                            <div class="progress-bar bg-success" role="progressbar"
                                                 ...
                                                 aria-valuemin="0" aria-valuemax="100" title="${porcentagem}%"></div>
                                        </div>
                                    </c:forEach>
                                </c:if>
                            </div>
                            ...
                            <div class="card-header py-3">
                                <h6 class="m-0 font-weight-bold text-danger">Enquetes Encerradas:
                                    <c:if test="${not empty categoriasFechada.content}">
                                        <br><small>Exibindo: ${categoriasFechada.content.size()}
                                        de ${categoriasFechada.totalElements}</small>
                                    </c:if>
                                ...
                                <c:if test="${not empty categoriasFechada.content}">
                                    <c:forEach var="categoria" items="${categoriasFechada.content}">
                                        <fmt:formatNumber var="porcentagem"
                                                          ...
                                                          maxFractionDigits="0"/>
                                        <h4 class="small font-weight-bold"><a
                                                href="${context}/dashboard/campanhas/${categoria.campanha.id}/categorias/${categoria.id}/resultados"
                                                class="text-danger">${categoria.campanha.nome}
                                            - ${categoria.tipo.descricao}</a>
                                            <span class="float-right">
                          <c:if test="${porcentagem ne 'NaN'}">Total de Votos: ${categoria.votos.size()}</c:if>
                          <c:if test="${porcentagem eq 'NaN'}">Total de Votos: 0</c:if>
